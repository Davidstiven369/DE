...
Maestroesfuerzo.getConsultByIdProyect = (id, result) => {
    ...
    sql.query(`SELECT * FROM digitalestimator.maestroesfuerzo WHERE id_proyecto = ?`, [id], (err, res) => {
        if (err) {
            console.log("error", err);
            result(err, null);
            return;
...
Maestroesfuerzo.getTwoo = (id) => {
    return new Promise((resolve, reject) => {
        sql.query(`
                            SELECT * FROM digitalestimator.esfuerzopivote WHERE id_tamanio = $ { id }
                            `, (err, res) => {
            if (err) {
                reject(err);
            }
...
Maestroesfuerzo.updateById = (horas, id, result) => {
    ...
    sql.query(
        "UPDATE digitalestimator.maestroesfuerzo SET total_horas_hombre_dev = ? WHERE id_maestroesfuerzo = ?;", [horas, id],
        (err, res) => {
            if (err) {
                result(null, err);
                return;
